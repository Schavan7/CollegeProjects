...
#Q1.
#1.
nfl1= read.csv("nfl1.csv", header = TRUE)
...
# plotted all variable to coss verify with excel and R summary  to come to a conclusion
plot(nfl1$x1)
plot(nfl1$x2)
plot(nfl1$x3)
plot(nfl1$x4)
...
anova(reducedModel) # this is correct
...
t.test(x8,alternative="two.sided",conf.level=0.95)
...
head(survey, 10)
#How does the frequency of mental health illness differ by workpace type (i.e., tech vs. non-tech) and size? We will explore using ggplot
...
#Can we test to see if no_employees and tech_company are statistically significant predictors of mental health condition?
# Use logistic regression

# transform into a factor variable
survey$tech_company <- as.factor(survey$tech_company)
logit <- glm(treatment ~ tech_company + no_employees, data = survey ) #family = "binomial"
summary(logit) 
# The estimates can be exponentiated to give us odds ratios 



exp(cbind(OR = coef(logit), confint(logit))) 
# Gives us odds ratios and accompanying 95% confidence intervals
#--------
